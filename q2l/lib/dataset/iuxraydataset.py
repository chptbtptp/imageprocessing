...
class IUXrayDataset(data.Dataset):
    ...
    def get_anno(self):
        self.img_list = json.loads(open(os.path.join(self.root,'iuxray_label_35_annotation.json'), 'r').read())[self.phase]
        self.cat2idx = json.load(open(os.path.join(self.root, 'iu_category_35.json'), 'r'))
